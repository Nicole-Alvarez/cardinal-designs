...
import { type TemplateCanvas, type TemplateContent } from "@/features/templates/types";
...
export default function AiCreateDialog({
  ...
  const [referenceMode, setReferenceMode] = useState<"match" | "inspiration">("match");
  ...
  async function generate() {
    ...
    try {
      const content = reference ? await createAiReferenceLayout(originalPrompt, canvas, reference, referenceMode, controller.current.signal) : await createAiLayout(originalPrompt, canvas);
      onApply(content);
      clearReference();
      onClose();
    } catch (cause) {
    ...
    <AccessibleDialog open={open} onClose={close} labelledBy="ai-create-title" panelClassName="w-full max-w-xl rounded-2xl bg-surface-1 p-5 text-text-primary shadow-2xl">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary">Describe a card layout. It will be applied directly to the canvas.</p>
        </div>
      ...
      <>
        ...
        {reference ? <div className="mt-3 flex rounded-lg border border-border-subtle p-1 text-sm"><button type="button" aria-pressed={referenceMode === "match"} onClick={() => setReferenceMode("match")} className={`rounded px-3 py-1 ${referenceMode === "match" ? "bg-surface-selected" : ""}`}>Match closely</button><button type="button" aria-pressed={referenceMode === "inspiration"} onClick={() => setReferenceMode("inspiration")} className={`rounded px-3 py-1 ${referenceMode === "inspiration" ? "bg-surface-selected" : ""}`}>Use as inspiration</button></div> : null}
        {error ? <p role="alert" className="mt-3 text-sm text-red-600">{error}</p> : null}
        <div className="mt-5 flex justify-end gap-2"><Button variant="ghost" onClick={close} disabled={loading}>Cancel</Button><Button onClick={() => void generate()} disabled={loading || (!originalPrompt.trim() && !reference)}>{loading ? "Generating…" : reference ? "Analyze and generate" : "Generate layout"}</Button></div>
      </>
    </AccessibleDialog>
  );
}
